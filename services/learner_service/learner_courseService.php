...
function getInstructorCourse($id)
{
    ...
    $sql = "select users.u_name,all_courses.course_name from learner_instructor join users join all_courses on learner_instructor.instructor_id=users.id and all_courses.course_id=learner_instructor.course_id where learner_instructor.learner_id='{$id}' ";
	$result = mysqli_query($conn, $sql);
	$courses = [];
	while($row = mysqli_fetch_assoc($result)){
		array_push($courses, $row);
